...
empty = coord.filter(lambda x: (x[1][0] == 0 or x[1][0] is None) and \
        (x[1][1] == 0 or x[1][1] is None) and \
        (x[1][2] == 0 or x[1][2] is None) and \
        (x[1][3] == 0 or x[1][3] is None))
...
result = coord.leftOuterJoin(empty) \
        .map(lambda x: (x[0], '%.2f' % (float(0 if x[1][1] is None else x[1][1]) / x[1][0] * 100))) \
        .sortBy(lambda x: x[0])
